...
- [ ] add openrouter API key + model selector
- [ ] add intermediary color scheme (add it to the switcher temporarily), in a new section, add controls for this separately in CSS with clear comments so I can play around with the settings
- [ ] Have an 'x minute read' right under the heading in a subtle font and size, depending on the word count
- [ ] Disable the Rerun LLM button while the LLM is generating content
- [ ] the loading spinner should not be scrollable (currently if i press the re-run LLM button, i can scroll down to see the previous generated text. it looks buggy.)
